...
const IMAGE = map.protocol.imageWords;
...
(async () => {
  ...
  const ranges = [...new Set([0, IMAGE, CMD, CMD + IMAGE, ST, ST + IMAGE])].filter((b) => b >= 0).sort((a, b) => a - b);
  for (const base of ranges) {
    try {
      const r = await client.readHoldingRegisters(base, IMAGE);
      console.log('Holding ' + base + '..' + (base + IMAGE - 1) + ':');
      block(r.data, base);
    } catch (e) {
  ...
  const limit = Math.max(1300, ST + IMAGE * 2);
  console.log('scan 0..' + limit + ' for 0x4C4D ...');
  const hits = [];
  for (let base = 0; base < limit; base += IMAGE) {
    try {
      const r = await client.readHoldingRegisters(base, Math.min(IMAGE, limit - base));
  ...
  if (hits.includes(ST)) {
    ...
    console.log('[probe] 状态区缺失：Holding ' + ST + '..' + (ST + IMAGE - 1) + ' 没有 0x4C4D。');
