...
(async () => {
  ...
  const required = ['AxisFb_rVelActM1', 'AxisFb_rVelActM2', 'AxisFb_rSyncErr', 'AxisFb_xSyncFault'];
  ...
  ws.on('message', (raw) => {
    ...
    if (m.t === 's') {
      got.status = true;
      if ('AxisFb_rVelActM1' in m && 'AxisFb_rVelActM2' in m && 'AxisFb_rSyncErr' in m) got.actFields = true;
    }
  });
